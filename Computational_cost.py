# -*- coding: utf-8 -*-
"""
The code runs optimization cases for wind farms with 8, 10, 12, and 16 turbines. 
To ensure a fair comparison, the initial boundary area is scaled in each case to maintain 
the same initial installed power density, while all other parameters remain constant. 
For each configuration, the results are saved for both the Baseline and Sequential strategies — 
most importantly, computation time.
...
if __name__ == '__main__':
    ...
    folder_name = "computational time" + datetime.now().strftime("(%Y-%m-%d %H-%M-%S)")
    with Save_output.save_output(folder_name) as saved_folder:
        ...
        # Minimum distance between turbines in terms of D
        min_dist_D = 4

        # AEP sacrifice
        aep_sacrifice = 0.0002

        # Define number of turbines to itterate over 
        num_turb = [8,10,12,14,16]

        # Array for for saving computational times
        baseline_comp_times = np.zeros(len(num_turb))
        seq_comp_times = np.zeros(len(num_turb))
        
        # Create a matrix (list of lists) to store all outputs
        all_results = all_results = [None] * len(num_turb)

        for i, N_t in enumerate(num_turb):
             call_fmodel = copy.deepcopy(fmodel)
             #boundary
             k = 1.15
             max_min_D=7 
             # Boundary length in terms of number of D- insure same instaled power density  
             L = k*np.sqrt(N_t)*max_min_D 
             boundaries = [(0.0, 0.0), (0.0, L * D), (L * D, L * D), (L * D, 0.0), (0.0, 0.0)]
             # Generate random starting layout
             x_initial, y_initial = Layout.generate_random_layout(N_t, boundaries, max_min_D, D)
             ...
             baseline_comp_times[i] = baseline_time
             seq_comp_times[i] = sequential_time
        ...
        to_save = {
            ...
            "number_of_turbines":num_turb,
            "baseline_comp_times":baseline_comp_times,
            "seq_comp_times":seq_comp_times
              }
